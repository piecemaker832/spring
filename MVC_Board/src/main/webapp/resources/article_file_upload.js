...
//		    return ctxPath;
		...
		fileDropDiv.on("dragenter dragover", function(evnet){
			event.preventDefault();
		});
		...
		function uploadFile(formData){
			$.ajax({
				url:getContextPath()+"/article/file/upload",
				data: formData,
				dataType : "text",
				// processData : 데이터를 일반적인 query string으로 변환처리 할 것인지 결정
				...
				// multiapart/form-data 방식으로 전송하기 위해 false
				contentType : false,
				type : "POST",
		...
		function deleteFileWrtPage(that) {
			var url = getContextPath()+"/article/file/delete";
			deleteFile(url, that);
		}
		...
		function getFileInfo(fullName){
			...
			if(checkImageType(fullName)) {
				imgSrc = getContextPath()+"/article/file/display?fileName="+fullName; // 썸네일 이미지 링크
				uuidFileName = fullName.substr(14);
				var originalImg = fullName.substr(0,12) + fullName.substr(14);
				// 원본 이미지 요청 링크
				originalFileUrl = getContextPath()+"/article/file/display?fileName=" + originalImg;
			} else {
				imgSrc = getContextPath()+"/resources/files/fileIcon.png" //파일 아이콘 이미지 링크
				uuidFileName = fullName.substr(12);
				//파일 다운로드 요청 링크
				originalFileUrl = getContextPath()+"/article/file/display?fileName=" + fullName;
			}
		...
		//파일목록 : 게시글 조회 / 수정페이지
		function getFiles(boardNo) {
			$.getJSON(getContextPath()+"/article/file/list/" + boardNo, function (list){
				if(list.length === 0) {
		...
		// 파일 삭제 AJAX 통신
		function deleteFileModPage(that,boardNo){
			var url = getContextPath() +"/article/file/delete"+boardNo;
			deleteFile(url,that);
		}
